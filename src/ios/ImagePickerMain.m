...
@interface ImagePicker ()<IDSImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate> {
...
-(void)takeVideo:(CDVInvokedUrlCommand *)command{
    ...
    self.maxCountTF = 1;
    IDSImagePickerController *imagePickerVc = [[IDSImagePickerController alloc] initWithMaxImagesCount:self.maxCountTF columnNumber:self.columnNumberTF delegate:(id<IDSImagePickerControllerDelegate>)self pushPhotoPickerVc:YES];
...
#pragma mark - 四类个性化设置，这些参数都可以不传，此时会走默认设置
    ...
    imagePickerVc.videoMaximumDuration = 15.f;
...
#pragma mark - 到这里为止
    [imagePickerVc setDidFinishPickingVideoHandle:^(UIImage *coverImage, id asset) {
        ...
        [[IDSImageManager manager] getVideoOutputPathWithAsset:asset completion:^(NSString *outputPath) {
            NSLog(@"视频导出到本地完成,沙盒路径为:%@",outputPath);
            //  Export completed, send video here, send by outputPath or NSData
            // 导出完成，在这里写上传代码，通过路径或者通过NSData上传
            
            NSString *fileName = [[[outputPath lastPathComponent] stringByDeletingPathExtension] stringByAppendingPathExtension:@"JPG"];
            NSData * data = UIImageJPEGRepresentation(coverImage, 1);
            NSString *path2= [self saveNSDataToFile:data withName:fileName];
            NSDictionary* result = @{@"images": @[@{@"path":outputPath,@"coverImage":path2}]};
...
- (void)saveOriginalImage:(NSArray *)assets currentIdx:(NSInteger)idx  originalPathArray:(NSMutableArray *)originalPaths completion:(void (^)(NSMutableArray *photos))completion {
    if([assets count] - 1 >= idx) {
        ...
        [[IDSImageManager manager] getOriginalPhotoDataWithAsset:asset completion:^(NSData *data, NSDictionary *info, BOOL isDegraded) {
...
- (void)saveCompressImage:(NSArray *)assets currentIdx:(NSInteger)idx compressedPathArray:(NSMutableArray *)compressedPaths completion:(void (^)(NSMutableArray *photos))completion {
    if([assets count] - 1 >= idx) {
        ...
        if([originExt isEqualToString:@"gif"]) {
            ...
            [[IDSImageManager manager] getOriginalPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info) {
                ...
                NSString *path = [self saveNSDataToFile:compressed withName:originName];
                
                NSString *path2= [self saveNSDataToFile:compressed withName:fileName];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    IDSTestCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IDSTestCell" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] containsString:@"GIF"] && self.enablePickingGif && !self.enablePickingMuitlpleVideo) {
            IDSGifPhotoPreviewController *vc = [[IDSGifPhotoPreviewController alloc] init];
            IDSAssetModel *model = [IDSAssetModel modelWithAsset:asset type:IDSAssetModelMediaTypePhotoGif timeLength:@""];
            vc.model = model;
            [self.viewController presentViewController:vc animated:YES completion:nil];
        } else if (isVideo && !self.enablePickingMuitlpleVideo) { // perview video / 预览视频
            IDSVideoPlayerController *vc = [[IDSVideoPlayerController alloc] init];
            IDSAssetModel *model = [IDSAssetModel modelWithAsset:asset type:IDSAssetModelMediaTypeVideo timeLength:@""];
            vc.model = model;
            [self.viewController presentViewController:vc animated:YES completion:nil];
        } else { // preview photos / 预览照片
            IDSImagePickerController *imagePickerVc = [[IDSImagePickerController alloc] initWithSelectedAssets:_selectedAssets selectedPhotos:_selectedPhotos index:indexPath.row];
...
- (void)pushTZImagePickerController {
    ...
    IDSImagePickerController *imagePickerVc = [[IDSImagePickerController alloc] initWithMaxImagesCount:self.maxCountTF columnNumber:self.columnNumberTF delegate:self pushPhotoPickerVc:YES];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) && iOS7Later) {
        ...
        // 拍照之前还需要检查相册权限
    } else if ([IDSImageManager authorizationStatus] == 2) { // 已被拒绝，没有相册权限，将无法保存拍的照片
        if (iOS8Later) {
    ...
    } else if ([IDSImageManager authorizationStatus] == 0) { // 未请求过相册权限
        [[IDSImageManager manager] requestAuthorizationWithCompletion:^{
            [self takePhoto];
        }];
    } else {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        IDSImagePickerController *tzImagePickerVc = [[IDSImagePickerController alloc] initWithMaxImagesCount:1 delegate:self];
        tzImagePickerVc.sortAscendingByModificationDate = self.enableSortAscending;
        [tzImagePickerVc showProgressHUD];
        UIImage *image = [info objectForKey:UIImagePickerControllerOriginalImage];

        // save photo and get asset / 保存图片，获取到asset
        [[IDSImageManager manager] savePhotoWithImage:image location:self.location completion:^(NSError *error){
            if (error) {
                [tzImagePickerVc hideProgressHUD];
                NSLog(@"图片保存失败 %@",error);
            } else {
                [[IDSImageManager manager] getCameraRollAlbum:NO allowPickingImage:YES  needFetchAssets:NO completion:^(IDSAlbumModel *model) {
                    [[IDSImageManager manager] getAssetsFromFetchResult:model.result allowPickingVideo:NO allowPickingImage:YES completion:^(NSArray<IDSAssetModel *> *models) {
                        [tzImagePickerVc hideProgressHUD];
                        IDSAssetModel *assetModel = [models firstObject];
                        if (tzImagePickerVc.sortAscendingByModificationDate) {
                            assetModel = [models lastObject];
                        }
                        if (self.enableCrop) { // 允许裁剪,去裁剪
                            IDSImagePickerController *imagePicker = [[IDSImagePickerController alloc] initCropTypeWithAsset:assetModel.asset photo:image completion:^(UIImage *cropImage, id asset) {
...
/// 用户点击了取消
- (void)tz_imagePickerControllerDidCancel:(IDSImagePickerController *)picker {
    NSLog(@"cancel");
...
// You can get original photo with asset, by the method [[IDSImageManager manager] getOriginalPhotoWithAsset:completion:].
// The UIImage Object in photos default width is 828px, you can set it by photoWidth property.
// 这个照片选择器会自己dismiss，当选择器dismiss的时候，会执行下面的代理方法
// 如果isSelectOriginalPhoto为YES，表明用户选择了原图
// 你可以通过一个asset获得原图，通过这个方法：[[IDSImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(IDSImagePickerController *)picker didFinishPickingPhotos:(NSArray *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto {
    _selectedPhotos = [NSMutableArray arrayWithArray:photos];
    _selectedAssets = [NSMutableArray arrayWithArray:assets];
    _isSelectOriginalPhoto = isSelectOriginalPhoto;
    [_collectionView reloadData];
...
- (void)imagePickerController:(IDSImagePickerController *)picker didFinishPickingVideo:(UIImage *)coverImage sourceAssets:(id)asset {
...
- (void)imagePickerController:(IDSImagePickerController *)picker didFinishPickingGifImage:(UIImage *)animatedImage sourceAssets:(id)asset {
