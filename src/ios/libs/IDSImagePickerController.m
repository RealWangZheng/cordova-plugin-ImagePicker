...
@interface IDSImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
@implementation IDSImagePickerController
...
- (void)viewDidLoad {
    ...
    [IDSImageManager manager].shouldFixOrientation = NO;
...
- (void)configBarButtonItemAppearance {
    ...
    if (iOS9Later) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[IDSImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[IDSImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<IDSImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<IDSImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<IDSImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    IDSAlbumPickerController *albumPickerVc = [[IDSAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[IDSImageManager manager] authorizationStatusAuthorized]) {
            ...
            if ([IDSImageManager authorizationStatus] == 0) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    IDSPhotoPreviewController *previewVc = [[IDSPhotoPreviewController alloc] init];
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [IDSImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [IDSImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[IDSImageManager manager] authorizationStatusAuthorized]) {
        ...
        IDSAlbumPickerController *albumPickerVc = (IDSAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[IDSAlbumPickerController class]]) {
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        IDSPhotoPickerController *photoPickerVc = [[IDSPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[IDSImageManager manager] getCameraRollAlbum:self.allowPickingVideo allowPickingImage:self.allowPickingImage needFetchAssets:NO completion:^(IDSAlbumModel *model) {
...
- (void)setPickerDelegate:(id<IDSImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [IDSImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [IDSImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [IDSImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (id asset in selectedAssets) {
        IDSAssetModel *model = [IDSAssetModel modelWithAsset:asset type:[[IDSImageManager manager] getAssetType:asset]];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [IDSImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [IDSImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [IDSImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [IDSImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [IDSImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
@interface IDSAlbumPickerController ()<UITableViewDataSource,UITableViewDelegate> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation IDSAlbumPickerController
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        IDSImagePickerController *imagePickerVc = (IDSImagePickerController *)self.navigationController;
        [[IDSImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage needFetchAssets:!self.isFirstAppear completion:^(NSArray<IDSAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumArr = [NSMutableArray arrayWithArray:models];
                for (IDSAlbumModel *albumModel in self->_albumArr) {
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[IDSAlbumCell class] forCellReuseIdentifier:@"IDSAlbumCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    IDSAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"IDSAlbumCell"];
    IDSImagePickerController *imagePickerVc = (IDSImagePickerController *)self.navigationController;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    IDSPhotoPickerController *photoPickerVc = [[IDSPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    IDSAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
@implementation IDSImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static IDSImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[IDSImagePickerConfig alloc] init];
