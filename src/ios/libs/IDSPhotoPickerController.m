...
#import "IDSPhotoPickerController.h"
#import "IDSImagePickerController.h"
#import "IDSPhotoPreviewController.h"
#import "IDSAssetCell.h"
#import "IDSAssetModel.h"
#import "UIView+IDSLayout.h"
#import "IDSImageManager.h"
#import "IDSVideoPlayerController.h"
#import "IDSGifPhotoPreviewController.h"
#import "IDSLocationManager.h"
#import <MobileCoreServices/MobileCoreServices.h>

@interface IDSPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate> {
...
@property (nonatomic, strong) IDSCollectionView *collectionView;
...
@implementation IDSPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (iOS9Later) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[IDSImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[IDSImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    IDSImagePickerController *tzImagePickerVc = (IDSImagePickerController *)self.navigationController;
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && iOS8Later && self->_model.isCameraRoll) {
            [[IDSImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:YES completion:^(IDSAlbumModel *model) {
                self->_model = model;
            ...
            if (self->_showTakePhotoBtn || !iOS8Later || self->_isFirstAppear) {
                [[IDSImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<IDSAssetModel *> *models) {
...
- (void)configCollectionView {
    ...
    _collectionView = [[IDSCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
    ...
    [_collectionView registerClass:[IDSAssetCell class] forCellWithReuseIdentifier:@"IDSAssetCell"];
    [_collectionView registerClass:[IDSAssetCameraCell class] forCellWithReuseIdentifier:@"IDSAssetCameraCell"];
}
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [IDSCommonTools tz_isIPhoneX] ? 50 + (83 - 49) : 50;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (iOS7Later && !isStatusBarHidden) top += [IDSCommonTools tz_statusBarHeight];
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        ...
        if (iOS7Later) navigationHeight += [IDSCommonTools tz_statusBarHeight];
    ...
    [IDSImageManager manager].columnNumber = [IDSImageManager manager].columnNumber;
...
- (void)previewButtonClick {
    IDSPhotoPreviewController *photoPreviewVc = [[IDSPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            IDSAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [IDSImageManager manager].shouldFixOrientation = YES;
        __block id alertView;
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            IDSAssetModel *model = tzImagePickerVc.selectedModels[i];
            [[IDSImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                if (isDegraded) return;
                if (photo) {
                    photo = [[IDSImageManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.row >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.row == 0)) && _showTakePhotoBtn) {
        IDSAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IDSAssetCameraCell" forIndexPath:indexPath];
    ...
    IDSAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IDSAssetCell" forIndexPath:indexPath];
    ...
    if (model.isSelected && tzImagePickerVc.showSelectedIndex) {
        NSString *assetId = [[IDSImageManager manager] getAssetIdentifier:model.asset];
    ...
    __weak IDSAssetCell *weakCell = cell;
    __weak IDSPhotoPickerController *weakSelf = self;
    __weak CALayer *weakLayer = _numberImageView.layer;
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        __strong IDSAssetCell *strongCell = weakCell;
        __strong IDSPhotoPickerController *strongSelf = weakSelf;
        __strong CALayer *strongLayer = weakLayer;
        IDSImagePickerController *tzImagePickerVc = (IDSImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (IDSAssetModel *model_item in selectedModels) {
                if ([[[IDSImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[IDSImageManager manager] getAssetIdentifier:model_item.asset]]) {
            ...
            [UIView showOscillatoryAnimationWithLayer:strongLayer type:IDSOscillatoryAnimationToSmaller];
        } else {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    IDSAssetModel *model = _models[index];
    if (model.type == IDSAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            IDSImagePickerController *imagePickerVc = (IDSImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both video and photo"]];
        } else {
            IDSVideoPlayerController *videoPlayerVc = [[IDSVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == IDSAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            IDSImagePickerController *imagePickerVc = (IDSImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            IDSGifPhotoPreviewController *gifPreviewVc = [[IDSGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied) && iOS7Later) {
        ...
        NSDictionary *infoDict = [IDSCommonTools tz_getInfoDictionary];
        // 无权限 做一个友好的提示
...
- (void)pushImagePickerController {
    // 提前定位
    __weak IDSPhotoPickerController *weakSelf = self;
    [[IDSLocationManager manager] startLocationWithSuccessBlock:^(NSArray<CLLocation *> *locations) {
        __strong IDSPhotoPickerController *strongSelf = weakSelf;
...
- (void)pushPhotoPrevireViewController:(IDSPhotoPreviewController *)photoPreviewVc {
    __weak IDSPhotoPickerController *weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[IDSImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.tz_width <= 320) {
        return;
    }
    IDSImagePickerController *imagePickerVc = (IDSImagePickerController *)self.navigationController;
    [[IDSImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    IDSImagePickerController *tzImagePickerVc = (IDSImagePickerController *)self.navigationController;
    for (IDSAssetModel *model in tzImagePickerVc.selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            [[IDSImageManager manager] savePhotoWithImage:photo location:self.location completion:^(NSError *error){
                if (!error) {
        ...
        if (videoUrl) {
            [[IDSImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(NSError *error) {
                if (!error) {
                    [self reloadPhotoArrayWithMediaType:type];
                }
            }];
...
- (void)reloadPhotoArrayWithMediaType:(NSString *)mediaType {
    IDSImagePickerController *tzImagePickerVc = (IDSImagePickerController *)self.navigationController;
    [[IDSImageManager manager] getCameraRollAlbum:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage needFetchAssets:NO completion:^(IDSAlbumModel *model) {
        self->_model = model;
        [[IDSImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<IDSAssetModel *> *models) {
            [tzImagePickerVc hideProgressHUD];
            
            IDSAssetModel *assetModel;
            if (tzImagePickerVc.sortAscendingByModificationDate) {
...
- (void)resetCachedAssets {
    [[IDSImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[IDSImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[IDSImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            IDSAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
...
@implementation IDSCollectionView
