...
@protocol IDSImagePickerControllerDelegate;
@interface IDSImagePickerController : UINavigationController
...
@property (nonatomic, strong) NSMutableArray<IDSAssetModel *> *selectedModels;
...
@property (nonatomic, weak) id<IDSImagePickerControllerDelegate> pickerDelegate;

@end


@protocol IDSImagePickerControllerDelegate <NSObject>
@optional
// The picker should dismiss itself; when it dismissed these handle will be called.
// You can also set autoDismiss to NO, then the picker don't dismiss itself.
// If isOriginalPhoto is YES, user picked the original photo.
...
// 你可以通过一个asset获得原图，通过这个方法：[[TZImageManager manager] getOriginalPhotoWithAsset:completion:]
// photos数组里的UIImage对象，默认是828像素宽，你可以通过设置photoWidth属性的值来改变它
- (void)imagePickerController:(IDSImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto;
- (void)imagePickerController:(IDSImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos;
...
@interface IDSAlbumPickerController : UIViewController
...
@interface IDSCommonTools : NSObject
+ (BOOL)tz_isIPhoneX;
...
@interface IDSImagePickerConfig : NSObject
