...
#import "IDSProgressView.h"

@interface IDSAssetCell ()
...
@implementation IDSAssetCell

- (void)setModel:(IDSAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[IDSImageManager manager] getAssetIdentifier:model.asset];
    }
    if (self.useCachedImage && model.cachedImage) {
        self.imageView.image = model.cachedImage;
    } else {
        int32_t imageRequestID = [[IDSImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            ...
            if ([self.representedAssetIdentifier isEqualToString:[[IDSImageManager manager] getAssetIdentifier:model.asset]]) {
    ...
    if (model.needOscillatoryAnimation) {
        [UIView showOscillatoryAnimationWithLayer:self.selectImageView.layer type:IDSOscillatoryAnimationToBigger];
    }
...
- (void)setType:(IDSAssetCellType)type {
    _type = type;
    if (type == IDSAssetCellTypePhoto || type == IDSAssetCellTypeLivePhoto || (type == IDSAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    } else if (type == IDSAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        if (![IDSImagePickerConfig  sharedInstance].showSelectedIndex && ![IDSImagePickerConfig sharedInstance].showPhotoCannotSelectLayer) {
            [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:IDSOscillatoryAnimationToBigger];
        }
        // 用户选中了该图片，提前获取一下大图
        [self fetchBigImage];
    } else { // 取消选中，取消大图的获取
...
- (void)fetchBigImage {
    _bigImageRequestID = [[IDSImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (self->_progressView) {
            [self hideProgressView];
        }
...
- (IDSProgressView *)progressView {
    if (_progressView == nil) {
        _progressView = [[IDSProgressView alloc] init];
...
@interface IDSAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation IDSAlbumCell
...
- (void)setModel:(IDSAlbumModel *)model {
    _model = model;
    ...
    [[IDSImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
@implementation IDSAssetCameraCell
