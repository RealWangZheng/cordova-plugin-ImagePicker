...
#import "IDSPhotoPreviewCell.h"
#import "IDSAssetModel.h"
#import "UIView+IDSLayout.h"
#import "IDSImagePickerController.h"
#import "IDSImageManager.h"
#import "IDSImageCropManager.h"

@interface IDSPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation IDSPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [IDSImageManager manager].shouldFixOrientation = YES;
    __weak IDSPhotoPreviewController *weakSelf = self;
    IDSImagePickerController *_tzImagePickerVc = (IDSImagePickerController *)weakSelf.navigationController;
...
- (void)configBottomToolBar {
    ...
    IDSImagePickerController *_tzImagePickerVc = (IDSImagePickerController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[IDSPhotoPreviewCell class] forCellWithReuseIdentifier:@"IDSPhotoPreviewCell"];
    [_collectionView registerClass:[IDSVideoPreviewCell class] forCellWithReuseIdentifier:@"IDSVideoPreviewCell"];
    [_collectionView registerClass:[IDSGifPreviewCell class] forCellWithReuseIdentifier:@"IDSGifPreviewCell"];
}
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop) {
        ...
        [IDSImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = [IDSCommonTools tz_statusBarHeight];
    ...
    CGFloat toolBarHeight = [IDSCommonTools tz_isIPhoneX] ? 44 + (83 - 49) : 44;
...
- (void)select:(UIButton *)selectButton {
    IDSImagePickerController *_tzImagePickerVc = (IDSImagePickerController *)self.navigationController;
    IDSAssetModel *model = _models[_currentIndex];
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (model.type == IDSAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
        ...
        for (IDSAssetModel *model_item in selectedModels) {
            if ([[[IDSImageManager manager] getAssetIdentifier:model.asset] isEqualToString:[[IDSImageManager manager] getAssetIdentifier:model_item.asset]]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    IDSAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:IDSOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:IDSOscillatoryAnimationToSmaller];
}
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop) { // 裁剪状态
        ...
        IDSPhotoPreviewCell *cell = (IDSPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
        UIImage *cropedImage = [IDSImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [IDSImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    IDSImagePickerController *_tzImagePickerVc = (IDSImagePickerController *)self.navigationController;
    IDSAssetModel *model = _models[indexPath.row];
    
    IDSAssetPreviewCell *cell;
    __weak IDSPhotoPreviewController *weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == IDSAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IDSVideoPreviewCell" forIndexPath:indexPath];
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == IDSAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IDSGifPreviewCell" forIndexPath:indexPath];
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"IDSPhotoPreviewCell" forIndexPath:indexPath];
        IDSPhotoPreviewCell *photoPreviewCell = (IDSPhotoPreviewCell *)cell;
        photoPreviewCell.cropRect = _tzImagePickerVc.cropRect;
        photoPreviewCell.allowCrop = _tzImagePickerVc.allowCrop;
        __weak IDSImagePickerController *weakTzImagePickerVc = _tzImagePickerVc;
        __weak UICollectionView *weakCollectionView = _collectionView;
        __weak IDSPhotoPreviewCell *weakCell = photoPreviewCell;
        [photoPreviewCell setImageProgressUpdateBlock:^(double progress) {
            __strong IDSPhotoPreviewController *strongSelf = weakSelf;
            __strong IDSImagePickerController *strongTzImagePickerVc = weakTzImagePickerVc;
            __strong UICollectionView *strongCollectionView = weakCollectionView;
            __strong IDSPhotoPreviewCell *strongCell = weakCell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[IDSPhotoPreviewCell class]]) {
        [(IDSPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[IDSPhotoPreviewCell class]]) {
        [(IDSPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[IDSVideoPreviewCell class]]) {
        [(IDSVideoPreviewCell *)cell pausePlayerAndShowNaviBar];
    }
}
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == IDSAssetModelMediaTypeVideo) {
    ...
    if (![[IDSImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)showPhotoBytes {
    [[IDSImageManager manager] getPhotosBytesWithArray:@[_models[_currentIndex]] completion:^(NSString *totalBytes) {
