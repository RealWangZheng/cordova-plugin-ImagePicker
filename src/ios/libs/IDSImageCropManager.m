...
@implementation IDSImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    cropedImage = [[IDSImageManager manager] fixOrientation:cropedImage];
...
+ (UIImage *)sd_tz_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [IDSImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 200;
