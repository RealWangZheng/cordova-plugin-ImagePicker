...
@interface IDSImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@property (nonatomic, strong) ALAssetsLibrary *assetLibrary;
@end

@implementation IDSImageManager

CGSize AssetGridThumbnailSize;
CGFloat IDSScreenWidth;
CGFloat IDSScreenScale;

static IDSImageManager *manager;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (IDSScreenWidth - 2 * margin - 4) / columnNumber - margin;
    AssetGridThumbnailSize = CGSizeMake(itemWH * IDSScreenScale, itemWH * IDSScreenScale);
}

- (void)configTZScreenWidth {
    IDSScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    IDSScreenScale = 2.0;
    if (IDSScreenWidth > 700) {
        IDSScreenScale = 1.5;
    }
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(IDSAlbumModel *model))completion {
    __block IDSAlbumModel *model;
    if (iOS8Later) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<IDSAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(id)result completion:(void (^)(NSArray<IDSAssetModel *> *))completion {
    IDSImagePickerConfig *config = [IDSImagePickerConfig sharedInstance];
...
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<IDSAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        [fetchResult enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            IDSAssetModel *model = [self assetModelWithAsset:obj allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
            if (model) {
        ...
        ALAssetsGroupEnumerationResultsBlock resultBlock = ^(ALAsset *result, NSUInteger index, BOOL *stop)  {
            ...
            IDSAssetModel *model = [self assetModelWithAsset:result allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
            if (model) {
                [photoArr addObject:model];
            }
        };
...
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(IDSAssetModel *))completion {
    if ([result isKindOfClass:[PHFetchResult class]]) {
        PHFetchResult *fetchResult = (PHFetchResult *)result;
        PHAsset *asset;
        @try {
        ...
        IDSAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (completion) completion(model);
    } else if ([result isKindOfClass:[ALAssetsGroup class]]) {
        ALAssetsGroup *group = (ALAssetsGroup *)result;
        if (allowPickingImage && allowPickingVideo) {
...
- (IDSAssetModel *)assetModelWithAsset:(id)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    IDSAssetModel *model;
    IDSAssetModelMediaType type = [self getAssetType:asset];
    if ([asset isKindOfClass:[PHAsset class]]) {
        if (!allowPickingVideo && type == IDSAssetModelMediaTypeVideo) return nil;
        if (!allowPickingImage && type == IDSAssetModelMediaTypePhoto) return nil;
        if (!allowPickingImage && type == IDSAssetModelMediaTypePhotoGif) return nil;
        ...
        NSString *timeLength = type == IDSAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
        timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
        model = [IDSAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    } else {
        if (!allowPickingVideo){
            model = [IDSAssetModel modelWithAsset:asset type:type];
            return model;
        }
        /// Allow picking video
        if (type == IDSAssetModelMediaTypeVideo) {
...
- (IDSAssetModelMediaType)getAssetType:(id)asset {
    IDSAssetModelMediaType type = IDSAssetModelMediaTypePhoto;
    if ([asset isKindOfClass:[PHAsset class]]) {
        PHAsset *phAsset = (PHAsset *)asset;
        if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = IDSAssetModelMediaTypeVideo;
        else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = IDSAssetModelMediaTypeAudio;
            ...
            if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
                type = IDSAssetModelMediaTypePhotoGif;
            }
        }
    } else {
        if ([[asset valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
            type = IDSAssetModelMediaTypeVideo;
        }
    }
    return type;
}
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ...
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
                if (model.type != IDSAssetModelMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
            ...
            if (model.type != IDSAssetModelMediaTypeVideo) dataLength += (NSInteger)representation.size;
            if (i >= photos.count - 1) {
                NSString *bytes = [self getBytesFromDataLength:dataLength];
                if (completion) completion(bytes);
            }
...
- (int32_t)getPhotoWithAsset:(id)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = IDSScreenWidth;
...
- (int32_t)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    if ([asset isKindOfClass:[PHAsset class]]) {
        CGSize imageSize;
        if (photoWidth < IDSScreenWidth && photoWidth < _photoPreviewMaxWidth) {
            imageSize = AssetGridThumbnailSize;
        } else {
            PHAsset *phAsset = (PHAsset *)asset;
            CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
            CGFloat pixelWidth = photoWidth * IDSScreenScale * 1.5;
            // 超宽图片
            if (aspectRatio > 1.8) {
        ...
        dispatch_async(dispatch_get_global_queue(0,0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (photoWidth == IDSScreenWidth || photoWidth == self->_photoPreviewMaxWidth) {
                    dispatch_async(dispatch_get_global_queue(0,0), ^{
...
/// Get postImage / 获取封面图
- (void)getPostImageWithAlbumModel:(IDSAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        id asset = [model.result lastObject];
        if (!self.sortAscendingByModificationDate) {
            asset = [model.result firstObject];
        }
        [[IDSImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (IDSAlbumModel *)modelWithResult:(id)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    IDSAlbumModel *model = [[IDSAlbumModel alloc] init];
