...
@implementation IDSAssetPreviewCell
...
@implementation IDSPhotoPreviewCell

- (void)configSubviews {
    self.previewView = [[IDSPhotoPreviewView alloc] initWithFrame:CGRectZero];
    __weak IDSPhotoPreviewCell *weakSelf = self;
    [self.previewView setSingleTapGestureBlock:^{
        __strong IDSPhotoPreviewCell *strongSelf = weakSelf;
...
@interface IDSPhotoPreviewView ()<UIScrollViewDelegate>

@end

@implementation IDSPhotoPreviewView
...
- (void)setModel:(IDSAssetModel *)model {
    ...
    if (model.type == IDSAssetModelMediaTypePhotoGif) {
        // 先显示缩略图
        [[IDSImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            self.imageView.image = photo;
            [self resizeSubviews];
            // 再显示gif动图
            [[IDSImageManager manager] getOriginalPhotoDataWithAsset:model.asset completion:^(NSData *data, NSDictionary *info, BOOL isDegraded) {
                if (!isDegraded) {
...
- (void)setAsset:(id)asset {
    ...
    _asset = asset;
    self.imageRequestID = [[IDSImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
@implementation IDSVideoPreviewCell
...
- (void)configMoviePlayer {
    ...
    [[IDSImageManager manager] getPhotoWithAsset:self.model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        self->_cover = photo;
    }];
    [[IDSImageManager manager] getVideoWithAsset:self.model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
@implementation IDSGifPreviewCell
...
- (void)configPreviewView {
    _previewView = [[IDSPhotoPreviewView alloc] initWithFrame:CGRectZero];
    __weak IDSGifPreviewCell *weakSelf = self;
    [_previewView setSingleTapGestureBlock:^{
        __strong IDSGifPreviewCell *strongSelf = weakSelf;
